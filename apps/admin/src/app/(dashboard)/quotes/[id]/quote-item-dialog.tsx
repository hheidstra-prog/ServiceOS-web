...
export function QuoteItemDialog({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingItem) {
        await updateQuoteItem(editingItem.id, quoteId, {
          description,
          quantity: qty,
          unitPrice: price,
          taxRate: tax,
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>
            {editingItem ? "Update the line item details." : "Add a new line item to this quote."}
          </DialogDescription>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex items-center space-x-2">
            ...
            <Label htmlFor="isOptional" className="font-normal">
              Optional item (client can choose to include or exclude)
            </Label>
          </div>
